...
def getAnchorSearchArea(x, y, anchor, blockSize, searchArea):
    ...
    # ensure search area is in bounds
    sx = max(0, cx-int(blockSize/2)-searchArea)
    # and get top left corner of search area
    sy = max(0, cy-int(blockSize/2)-searchArea)

    # slice anchor frame within bounds to produce anchor search area
    anchorSearch = anchor[sy:min(
        sy+searchArea*2+blockSize, h), sx:min(sx+searchArea*2+blockSize, w)]
...
def getBlockZone(p, aSearch, tBlock, blockSize):
    ...
    try:
        ...
        print(e)
        print(
            f"ERROR - ABLOCK SHAPE: {aBlock.shape} != TBLOCK SHAPE: {tBlock.shape}")
...
def getBestMatch(tBlock, aSearch, blockSize):  # 3 Step Search
    ...
    while step >= 1:
        ...
        pointList = [p1, p2, p3, p4, p5, p6, p7,
                     p8, p9]  # retrieve 9 search points

        for p in range(len(pointList)):
            # get anchor macroblock
    ...
    px, py = px - int(blockSize / 2), py - int(blockSize /
                                               2)  # get top left corner of minP
...
def blockSearchBody(anchor, target, blockSize, searchArea=7):
    ...
    for y in range(0, int(hSegments*blockSize), blockSize):
        for x in range(0, int(wSegments*blockSize), blockSize):
            bcount += 1
            targetBlock = target[y:y+blockSize, x:x +
                                 blockSize]  # get current macroblock

            anchorSearchArea = getAnchorSearchArea(
                x, y, anchor, blockSize, searchArea)  # get anchor search area

            #print("AnchorSearchArea: ", anchorSearchArea.shape)

            # get best anchor macroblock
            anchorBlock = getBestMatch(
                targetBlock, anchorSearchArea, blockSize)
    ...
    # check all macroblocks are accounted for
...
def showImages(*kwargs):  # shows images
    ...
    for k in range(n):
        ...
        plt.imshow(kwargs[k])
        if i == 1:
            plt.title("Frame to be encoded")
        elif i == 2:
            plt.title("Reference frame")
        else:
            plt.title("Residue frame")
        i += 1
    plt.show()
...
def preprocess(anchor, target, blockSize):
    ...
    if isinstance(anchor, str) and isinstance(target, str):
        anchorFrame = BGR2YCrCb(cv2.imread(anchor))[
            :, :, 0]  # get luma component
        targetFrame = BGR2YCrCb(cv2.imread(target))[
            :, :, 0]  # get luma component
    ...
    anchorFrame = cv2.resize(
        anchorFrame, (int(wSegments*blockSize), int(hSegments*blockSize)))
    targetFrame = cv2.resize(
        targetFrame, (int(wSegments*blockSize), int(hSegments*blockSize)))
...
def main(anchorFrame, targetFrame, outfile="OUTPUT", saveOutput=True, blockSize=16):
    ...
    anchorFrame, targetFrame = preprocess(
        anchorFrame, targetFrame, blockSize)  # processes frame or filepath to frame
    ...
    reconstructTargetFrame = getReconstructTarget(
        residualFrame, predictedFrame)
    ...
    if saveOutput:
        ...
        cv2.imwrite(f"{outfile}/reconstructTargetFrame.png",
                    reconstructTargetFrame)
        cv2.imwrite(f"{outfile}/naiveResidualFrame.png", naiveResidualFrame)
        resultsFile = open(f"{outfile}/results.txt", "w")
        resultsFile.write(f"{rmText}\n{nrmText}\n")
        resultsFile.close()
...
if __name__ == "__main__":
    ...
    ANCHOR_FRAME = os.path.join("..", "data", "city001.png")
    TARGET_FRAME = os.path.join("..", "data", "city002.png")
    main(ANCHOR_FRAME, TARGET_FRAME)
